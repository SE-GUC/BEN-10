  ...
  postRequest () {
    ...
    test(`post ${this.base_url}`, async () => {
     ...
     expect(requestBody.description).toEqual(event.description);
      ...
      expect(new Date(requestBody.eventDate)).toEqual(new Date(event.eventDate));
      ...
      expect(new Date(requestBody.regist_start_date)).toEqual(new Date(event.regist_start_date));
      expect(new Date(requestBody.regist_expiry_date)).toEqual(new Date(event.regist_expiry_date));
      expect(requestBody.requestorId.toString()).toEqual(event.requestorId.toString());
...
// get the posted event recently by its id
  getRequestById  () {
     test("fetching a specific event", async () => {
      ...
      expect(jsonResponse.data.requestedBy).toEqual(this.sharedState.requestedBy)
      ...
      expect(jsonResponse.data.eventLocation).toEqual(this.sharedState.eventLocation)
      expect(new Date(jsonResponse.data.eventDate)).toEqual(this.sharedState.eventDate)
      expect(jsonResponse.data.registPrice).toEqual(this.sharedState.registPrice)
      expect(jsonResponse.data.remainingPlace).toEqual(this.sharedState.remainingPlace)
      expect(jsonResponse.data.topics.toString()).toEqual(this.sharedState.topics.toString())
  ...
  putRequest  () {
    const requestBody={
      description:"the previous project was aimed for software developpers only here this event welcomes everybody",
      eventLocation:"maadi"
    }
