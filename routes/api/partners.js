...
router.put('/:id1/AssignCAtoProject/:id2',async (req,res) => {
    ...
    if(ObjectId.isValid(projID) && ObjectId.isValid(caId)){
        ...
        if (project && consultancy){
           ...
           console.log(consul)
           var found =consul.includes(ObjectId(consultancy));
           console.log(found)
           var need = project.want_consultancy;
           console.log(need)
        if(need == true){ 
            if(found==true){
             const j=await assigning(caId,consultancy.name,projID);
...
async function assigning(caID,name,projID){
    var error = true;
    const body = { consultancyID: caID , consultancy:name };
     var j;
