...
router.put('/:id', async (req,res) => {
    try {
        if(ObjectId.isValid(req.params.id))
            {const updatedApplication = await Application.findByIdAndUpdate({_id: req.params.id}, req.body)
            if (!updatedApplication) return res.status(404).send({error: 'Application does not exist'})
 ...
 router.delete('/:id', async (req,res) => {
    try {
        if(ObjectId.isValid(req.params.id)){
            ...
            if(!deletedApplication) return res.status(400).send({ error: 'Application does not exist'})
